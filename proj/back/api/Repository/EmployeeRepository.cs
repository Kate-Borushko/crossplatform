...
namespace api.Repository
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public async Task<List<Employee>> GetAllAsync(QueryObjectEmployee query)
        {
            var employees = _context.Employees.AsQueryable();

            if (!string.IsNullOrWhiteSpace(query.Surname))
            {
                employees = employees.Where(s => s.Surname.Contains(query.Surname));
            }

            if (!string.IsNullOrWhiteSpace(query.Position))
            {
                employees = employees.Where(s => s.Position.Contains(query.Position));
            }
            ...
            if (!string.IsNullOrWhiteSpace(query.SortBy))
            {
                ...
                {
                    employees = query.IsDescending ? employees.OrderByDescending(n => n.Surname) : employees.OrderBy(n => n.Surname);
                }
            }
