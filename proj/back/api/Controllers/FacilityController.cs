...
namespace api.Controllers
{
    ...
    public class FacilityController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetAll([FromQuery] QueryObjectFacility query)
        {
        ...
        public async Task<IActionResult> Create([FromRoute] int employeeId, decimal pressureTop, CreateFacilityDto facilityDto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            ...
            const decimal minPressure = 0.1m;
            var warnings = new List<string>();

            if(facilityDto.PressureTop < minPressure)
            {
                warnings.Add($"PressureTop should be at lest {minPressure}.");
            }
            ...
            var result = new
            {
                Data = facilityModel.ToFacilityDto(),
                Warnings = warnings.Any() ? warnings : null
            };
            return CreatedAtAction(nameof(GetById), new { id = facilityModel.Id }, result);
        }
